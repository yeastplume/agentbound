...
One `sendmsg` = one operation: canonical JSON `{"v":"agentbound.gateway.v0.1","operation_id":..., "operation":"git.push_staging", "args":{...}, "payload_sha256":..., "payload_len":n}` optionally followed in the same packet by the raw payload bytes after a `\n` separator (bundle ≤ `gateway.budgets.bytes_per_operation`, default 8 MiB; SEQPACKET max message size raised via `SO_SNDBUF`/`SO_RCVBUF` — verify on VM; fall back to chunked `payload_chunk` operations with a per-connection reassembly bound if the kernel limit bites, recording the deviation). Reply: one packet, canonical JSON `{ok, class, body|rule, operation_seq, trace_id}`. Caller-supplied `trace_id` is ignored unless equal to the record's.

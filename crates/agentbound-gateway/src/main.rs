...
impl Gateway {
    ...
    fn handle_packet(&mut self, i: usize) -> bool {
        ...
        if let Err((class, rule, detail, close)) = outcome {
            ...
            self.emit(if rule == "process_mismatch" { "gateway.process_mismatch" } else if rule == "descriptor_transfer" { "gateway.descriptor_transfer_rejected" } else { "gateway.operation_denied" }, "deny", &cr, Value::obj(vec![("class", Value::s(class)), ("credential_pid", Value::Int(self.conns[i].last_cred_pid as i64)), ("detail", Value::s(&detail)), ("establishing_pid", Value::Int(self.conns[i].inst.pid as i64)), ("rule", Value::s(rule))]));

...
|---|---|---|---|
| [Phase 1 normative requirements](phase-1-requirements.md) | 0.8 | frozen (WP0) | complete; all questions answered |
...
| [Session lifecycle and failure states](session-lifecycle.md) | 0.6 | frozen (WP0) | complete; one WP1 item (LC-2) |
| [Execution-identity lifecycle](execution-identity-lifecycle.md) | 0.6 | frozen (WP0) | complete; one WP1 item (ID-1) |
...
| [Open-question disposition register](open-question-register.md) | 0.4 | frozen (WP0) | all 32 WP0 questions disposed: 28 answered in their owning documents, 4 assigned to named WP1 items |
...
| [ADR-0002](ADR-0002-gateway-authentication.md) gateway topology and authentication | 0.6 | **accepted for Phase 1**, conditional on the Decision 7 kernel-baseline verification in WP1 | mechanism complete; one connection per process |
| [ADR-0003](ADR-0003-control-substrate.md) control substrate | 0.8 | **accepted for Phase 1** | configuration, thresholds, confidence method, and classification rules filled; image digests recorded in `pinned-configuration.json` at image build; per-ID `control-arm-register.md` is a 1D prerequisite; two WP1 items (VM-1, VM-2) |

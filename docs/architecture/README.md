...
|---|---|---|---|
| [Phase 1 normative requirements](phase-1-requirements.md) | 0.6 | frozen (WP0) | complete; all questions answered |
...
| [Session lifecycle and failure states](session-lifecycle.md) | 0.5 | frozen (WP0) | complete; one WP1 item (LC-2) |
| [Execution-identity lifecycle](execution-identity-lifecycle.md) | 0.5 | frozen (WP0) | complete; one WP1 item (ID-1) |
| [Component interfaces (skeleton)](component-interfaces.md) | 0.2 | frozen (WP0); security-relevant decisions | wire formats and versioning negotiation deferred to WP1 by design |
| [Test catalogue](test-catalogue.md) | 0.5 | frozen (WP0); pre-registration | load profiles, deadlines, seeds fixed; fixtures and commands are WP1 outputs by design |
| [Invariant-to-test traceability matrix](traceability-matrix.md) | 0.5 | frozen (WP0) | complete; requirement-coverage check performed (§8) |
| [Open-question disposition register](open-question-register.md) | 0.3 | frozen (WP0) | all 32 WP0 questions disposed: 28 answered in their owning documents, 4 assigned to named WP1 items |
...
| [ADR-0002](ADR-0002-gateway-authentication.md) gateway topology and authentication | 0.5 | **accepted for Phase 1**, conditional on the Decision 7 kernel-baseline verification in WP1 | mechanism complete; one connection per process |
| [ADR-0003](ADR-0003-control-substrate.md) control substrate | 0.6 | **accepted for Phase 1** | configuration, thresholds, confidence method, and classification rules filled; image digests recorded in `pinned-configuration.json` at image build; per-ID `control-arm-register.md` is a 1D prerequisite; two WP1 items (VM-1, VM-2) |
...
WP1 verification items are recorded in the [open-question register](open-question-register.md) and the owning document; a failed verification reopens the owning ADR before WP2 begins.

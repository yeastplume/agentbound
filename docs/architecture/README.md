...
|---|---|---|---|
| [Phase 1 normative requirements](phase-1-requirements.md) | 0.5 | frozen (WP0) | complete; all questions answered |
| [Authorization manifest and launch binding schema](manifest-schema.md) | 0.5 | frozen (WP0) | complete, with gateway-free 1A form and worked examples; all questions answered |
...
| [Test catalogue](test-catalogue.md) | 0.4 | frozen (WP0); pre-registration | load profiles, deadlines, seeds fixed; fixtures and commands are WP1 outputs by design |
| [Invariant-to-test traceability matrix](traceability-matrix.md) | 0.4 | frozen (WP0) | complete; requirement-coverage check performed (§8) |
| [Open-question disposition register](open-question-register.md) | 0.2 | frozen (WP0) | all 32 WP0 questions disposed: 28 answered in their owning documents, 4 assigned to named WP1 items |
...
**Freeze record.** Both freeze conditions were met at commit `fae6912` (no CERTAIN-class defect in the third independent review; every open question answered or assigned to a WP1 item in the [open-question register](open-question-register.md)). The WP1 verification set is ADR-0002 Decision 7 plus register items VM-1, VM-2, LC-2, ID-1; a failed item reopens the owning ADR or specification before the constructor is built.

...
- **0.11** — R-AUD-2 milestone split, and the §12 attribution row with it, following the first faithful measurement of the test-catalogue §5 NOMINAL profile in WP3.1. The gateway-operation obligation is unchanged and is met at 1B (100% over the finite corpus, ten seeded repetitions). The local-object and process-lifecycle classes move to **1C**, because the implementation has no ingestion path for them and building one — an audit-netlink shipper, per-session rule lifecycle, and reconciliation with R-AUD-3's host-global `lost` counter — is a work package rather than a repair. **No threshold is weakened:** ≥ 99% over the full three-class ontology remains the 1C bar and the Gate 3 condition, the measured 1B figure (4.3% overall) is retained as evidence rather than discarded, and 1B may not claim full-ontology attribution. This is a scope decision made explicitly and recorded, not a redefinition of a metric to make a failing result pass.
...
**R-AUD-2 (1B/1C) [Inv 13].** For the defined effect ontology (local objects in the session's world, process lifecycle events, gateway operations), `agentbound-audit` MUST reconstruct `initiator → agent → session → process → effect` and the report MUST state the fraction reconstructed, computed by the metric definition and load profiles in the [test catalogue](test-catalogue.md), against the thresholds in §12. The milestone split is:

- **1B.** The finite **gateway-operation** corpus MUST reconstruct at 100% under the §5 NOMINAL profile, including denied operations, matched to the workload's ground truth by class, outcome **and** idempotency key. A 1B report MUST state the measured whole-ontology fraction as well, and MUST NOT present the gateway figure as whole-ontology attribution.
- **1C.** The whole ontology MUST reach the §12 threshold. Until an ingestion path for local-object and process-lifecycle effects exists, this is not satisfiable, and a 1C report MUST state the fraction per class rather than only the aggregate.

Rationale for the split, recorded because it narrows a milestone's scope: WP3.1 measured the NOMINAL profile faithfully for the first time and found the gateway class at 100% and the other two at 0% — nothing in the implementation was ever built to collect them. Both earlier registers had scored the row by a presence check over event kinds already being emitted, which is why neither noticed.
...
| Attribution completeness | **1B:** 100% over the finite gateway-operation corpus per run, with the whole-ontology fraction also reported. **1C:** ≥ 99% correct full-chain reconstructions / in-scope ground-truth effects under the catalogue's nominal profile, plus 100% over the finite gateway-operation corpus per run; overload profile reported with loss counters; denominator, dedup, and correlation deadline per test catalogue | R-AUD-2 |

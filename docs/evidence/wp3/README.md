...
> **Status correction (independent review, 5 September 2026).** This register as first committed (`bd7befc`) overclaimed. The independent review of that commit found: (a) the runner has no completeness check against the frozen catalogue, so "139/139" counts the assertions this runner happened to emit, not the catalogue's 1B population — 29 frozen 1A/1B IDs have no machine row, 10 of them 1B-relevant (D-16, T-6.3-005, T-6.3-008, T-6.8-008, T-6.8-009, T-6.9-008, F-C-08, F-T-01 in its 1B form, F-T-06, F-T-07, F-T-09); (b) several executed rows are false positives or test a different mechanism than named — T-6.3-004 passes unconditionally, T-6.9-002 treats a failed measurement as 0, T-6.4-008's forged packets are refused at the establishment UID check so the per-packet `credential_count` branch was never exercised (and `requirement_for` maps a rule name that does not exist), T-6.4-013 fails at JSON parsing before the adapter and the assertion accepted any rule, T-6.4-009 draws on cumulative ordinary PID mismatches rather than the inode branch, T-6.4-004 has no bound host abstract-socket positive control, the new-connection half of T-6.4-014 is refused at the unrelated UID gate, D4.7-reconstruct can pass with no reconstruction evidence; (c) the constructor copies every manifest-`enforced` resource class into the signed binding as `installed_value` without installing `io.max` or a tmpfs `nr_inodes` bound; (d) gateway budget counters reset to zero on reconstruction and `bytes_used` is accumulated but never checked; (e) the pre-registered D-12 metric (8 sessions × 230 effects × 10 seeded repetitions, deadlines, denied operations, ≥ 99 % / 100 % gateway) was replaced by a 15-kind presence check; (f) the ten seeded repetitions were not run; (g) several rows depend on cumulative VM state and machine output lacks commit/binary digests.
>
> **Corrected status.** WP3 *implementation* complete enough for review. WP3 *conformance exit condition* **not met**. **Gate 3: not evaluated against the frozen exit criteria** — not a pass, provisional or otherwise. The row register below is retained as the record of what was run, with the findings above applying to it; the headline counts are restated in "WP3 exit status". Correction is scheduled as **WP3.1** (plan 0.16), a hard go/no-go gate before WP4. Nothing in §1 (components), §3 (one traced effect), §5 (SLOC), or the power-loss and D-Bus records is withdrawn, but each is a demonstration of a mechanism, not conformance evidence.
...
**Not met.** Final machine run: 139 assertions emitted, 139 passed, 0 failed — of which 4 are fixtures/markers (`PROBE-COMPLETE`, `T-6.8.setup`, `GW-HELD`, `GW-COMPLETE`) that should not have been counted. Restated by class after review: **strong pass** ≈ 118 (subject to the per-run scoping fix), **weak pass** 5 (T-6.9-006, T-6.4-009, T-6.4-012 and the two named in §2), **partial/recorded** 5 (D-02.1B, T-6.1-003.1B, T-6.3-008, T-6.2-008.1B, D-15.1B), **false positive / wrong mechanism** ≥ 8 (T-6.3-004, T-6.9-002, T-6.4-004, T-6.4-008, T-6.4-009, T-6.4-013, T-6.4-014 new-connection half, D4.7-reconstruct), **not executed** 29 catalogue IDs (10 1B-relevant), **fail** 0. Gate 3: **not evaluated**. The exact per-row reclassification is WP3.1's first deliverable, produced by a runner that checks itself against the frozen catalogue.
...
Recorded in [ADR-0002 0.9](../../architecture/ADR-0002-gateway-authentication.md). **Gate 3 verdict (superseded by the status correction above): not evaluated against the frozen criteria.** Items 6 and 8 in particular rest on rows the review found not to exercise the named mechanism (T-6.4-008/009/013) or not to have run (T-6.8-008/009, F-T-06/07/09).

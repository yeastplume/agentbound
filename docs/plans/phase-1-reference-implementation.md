...
- **0.17** — WP3.1 items 1–6 complete; go/no-go recorded against the five exit conditions and three stop-or-narrow triggers ([WP3.1 register](../evidence/wp3.1/README.md)). **Outcome: no-go to WP4, and narrow milestone 1B.** Conditions 1–4 are met (0 NOT-EXECUTED over the frozen catalogue; `installed_value` read back from the kernel or recorded `absent`; budget consumption restored after gateway restart; 48 fault-injection rows with no authority or identity survival). Condition 5 is not met and its second half — adversarial assertions owned by someone other than the implementation author — cannot be closed by the author. **R-AUD-2 is not satisfied at 1B:** measured attribution completeness is 3.5 % against a required ≥ 99 %, because the local-object and process-lifecycle effect classes have no ingestion path at all; WP2 and WP3 missed this because D-12 was scored by a presence check. Negative controls (new, item 6) found three rows that did not discriminate, including one passing on earlier runs' audit history. The R-CON-8 figure published at WP3.1 round 4 used the wrong scope and is corrected: direct privileged SLOC is **1 205 of 6 000** under the pinned `tokei 13.0.0-alpha.8`. Recommendations: re-scope R-AUD-2 to a dedicated work package or restate it as gateway-operation attribution only; treat `agentbound-lifecycle`'s serialization of blocking work as a §3.6 conformance defect; specify the in-session gateway protocol, which appears in no frozen document; adopt the negative-control harness as a release gate. Items 5 (ten seeded repetitions) and 7 (fresh-host reproduction) remain open.
- **0.16** — Independent review of the WP3 register (same day): the "139/139" claim counted runner assertions, not the frozen catalogue population (29 IDs unexecuted, 10 1B-relevant); ≥ 8 rows false-positive or testing a different mechanism (T-6.3-004, T-6.9-002, T-6.4-004, T-6.4-008, T-6.4-009, T-6.4-013, T-6.4-014, D4.7); constructor attests `installed_value` for I/O-bandwidth and inode limits it never installs; gateway budgets reset on restart; D-12 metric and ten-repetition rule not executed. WP3 status changed to *implementation complete, conformance exit not met, Gate 3 not evaluated*. **WP3.1** added as a hard go/no-go gate before WP4 with the correction list and the reviewer's five go conditions.
...
**Outcome (as recorded at 0.15, superseded at 0.16 — implementation complete, conformance exit NOT met, Gate 3 not evaluated; see WP3.1):** `agentbound-gateway` (ADR-0002 D1–D5, unprivileged, 317 SLOC; authentication path 107 SLOC), the `git.push_staging` adapter (bundle → quarantine → fsck/limits → `refs/agentbound/<session>/…` with the GS-4 refusal set, trace as push option, force never granted), `local-socket` constructor/manifest support, lifecycle §5 deny-admission and zero-connections steps, storage-principal projection, closed audit schemas for every gateway event. Final run **139 rows PASS, 0 FAIL** ([WP3 evidence register](../evidence/wp3/README.md)); D-13 shows the staging ref on the host with the session's trace in the host hook log while `main` is untouched; D-12 shows all 15 required event kinds on one launch record. Gate 3 recorded pass (ADR-0002 0.9). Carry-ins closed including a real power-loss round. Recorded limits: no TLS upstream (1C), no attach interface, no delegation path, weak same-tick PID-reuse evidence. **Stop point:** independent review of the register and trust boundary before WP4.

### WP3.1 — Conformance correction and independent test (hard gate before WP4)

Added at 0.16 after independent review of the WP3 register. Scope, in order:

1. **Harness integrity.** A checked-in expected-ID manifest derived from the frozen §6/§7.3/§5 catalogue for milestones 1A+1B; the runner MUST fail on missing, duplicate or substituted IDs and on partial/recorded verdicts the catalogue does not allow; fixtures and completion markers excluded from counts; every assertion scoped to the current run's launch records; exact `class` + `rule` asserted for every denial; machine output records commit, binary digests, run id, seed.
2. **False-positive repair.** T-6.3-004, T-6.9-002, T-6.4-004 (bind a real host abstract socket as positive control), T-6.4-008 (malformed `SCM_CREDENTIALS` from an in-scope session-UID process), T-6.4-009 (genuine pidfs-inode mismatch), T-6.4-013 (canonical args reaching the adapter), T-6.4-014 new-connection half, D4.7 (reconstruction evidence required).
3. **Implementation defects.** Gateway budgets persisted (lifecycle record store) and reconstructed atomically; per-session total bytes checked; `io.max` and tmpfs `nr_inodes` installed and **read back from the kernel** into the binding, or the class recorded `absent` — `installed_value` MUST never be copied from manifest intent (R-RES-2).
4. **Missing rows.** D-16, T-6.3-005, T-6.3-008, T-6.8-008, T-6.8-009, T-6.9-008, F-C-08, F-T-01 (1B), F-T-06, F-T-07, F-T-09; gateway construction/termination fault injection at every step the catalogue names.
5. **Pre-registered measurements.** D-12 as specified (8 sessions × 230 effects × 10 seeded repetitions, correlation deadlines, denied operations, ≥ 99 % overall / 100 % gateway); ten seeded repetitions of the suite.
6. **Negative controls.** Deliberately broken builds (inode comparison disabled, credential count disabled, scope check disabled, budget check disabled, …) each proving its assigned row fails.
7. **Independent reproduction.** Fresh provisioning of a second VM from `deploy/` and a full run; adversarial assertions owned and reviewed by someone other than the implementation author.

**Outcome (recorded at 0.17 — items 1–6 complete; NO-GO to WP4; narrow 1B):** the conformance corrections hold and WP3's exit condition does not. Conditions 1–4 met; condition 5 not met (item 7 not performed, and its independent-ownership half cannot be satisfied by the implementation author). **R-AUD-2 fails at 1B**: measured attribution completeness 3.5 % vs ≥ 99 % required, because two of the three §5 effect classes are never collected — a design gap, not a defect, and a work package rather than a repair. Negative controls found three non-discriminating rows (one passing on prior runs' audit history), all repaired; the credential-count branch is unreachable on the pinned kernel and ADR-0002 0.10 records it. R-CON-8 restated with the pinned tool at **1 205 / 6 000** direct privileged SLOC (the round-4 figure of 2 417 used the wrong scope). Full verdict and evidence: [WP3.1 register](../evidence/wp3.1/README.md).

Exit / go to WP4 only if: all mandatory 1B rows present; no claimed-enforced resource merely copied into the binding; budget enforcement survives restart; no missing fault point permits authority or identity reuse; an independent fresh-host run reproduces the result. **Stop or narrow** if the corrections require changing frozen semantics broadly, negative controls reveal broad false positives, or the direct privileged SLOC / lifecycle complexity grows enough to undermine the Unix-principal simplicity argument.
